...
{
	int MX_SIZE;
	for(int size_setter=0; size_setter<=8; size_setter++){
		float bintime_tot = 0;
		MX_SIZE = 32*(pow(2, size_setter));								// Takes matrix size  
		size_t r, c, rk, ck;
		...
		for(int tloop = 0; tloop<TEST_LOOP; tloop++){
			...
			for(int i = 0; i < r-rk+1; i++){
				for(int j = 0; j < c-ck+1; j++){
					...
					//		   2*(__builtin_popcount(~(tbA^bkerA))) - 48
					pC[i][j] = (__builtin_popcount(~(tbA^bkerA)));
				}
			}
		}	
		dTimeE = dsecnd();
